...
ps: 获取活动窗口焦点(很重要)，不管是press还是hotkey，在调用方法之前要思考当前鼠标焦点在哪
...
## 一，tkinter模块
### 1,1 常用函数
tk = tkinter.Tk()    创建并返回一个图形窗口对象
tk.title()                 修改标题
tk.maxsize             返回启动程序电脑的最大分辨率
tk.attributes("-topmost",True)     置顶窗口
tk.geometry(300*300+300+300)          设置界面窗口初始位置及自身窗口尺寸

tk.mainloop()            mainloop并不是运行一下就结束，里面是一个while True的死循环，函数体里不停地执行界面刷新和鼠标点击
tk.destroy ()              销毁窗口
tk.after()                    在窗口运行期间，预约一个未来的动作
```python
tk.after
```
...
tk.protocol()          
```python
# WM为window manager,关闭窗口时执行自己定义的回调函数
tk.protocol("WM_DELETE_WINDOW",回调函数)
```
...
## 二，dotenv模块
管理环境变量，dot是点的意思，因为一般以.开头的文件都是隐藏文件，就比如.env,所以名字为dotenv
